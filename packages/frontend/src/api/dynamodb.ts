	...
	return useMutation({
		mutationFn: (indexName: string) =>
			apiClient.delete<{ success: boolean }>(
				`/dynamodb/${tableName}/indexes/${indexName}`,
			),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["dynamodb", "table", tableName],
...
export function useScanItems(tableName: string) {
	return useMutation({
		mutationFn: (options?: ScanOptions) =>
			apiClient.post<ItemsResponse>(
				`/dynamodb/${tableName}/items/scan`,
				options ?? {},
			),
	});
}
...
export function useQueryItems(tableName: string) {
	return useMutation({
		mutationFn: (options: QueryOptions) =>
			apiClient.post<ItemsResponse>(
				`/dynamodb/${tableName}/items/query`,
				options,
			),
	});
}
...
export function useGetItem(tableName: string) {
	return useMutation({
		mutationFn: (key: Record<string, unknown>) =>
			apiClient.post<ItemsResponse>(`/dynamodb/${tableName}/items/get`, {
				key,
			}),
	});
}
...
export function usePutItem(tableName: string) {
	...
	return useMutation({
		mutationFn: (item: Record<string, unknown>) =>
			apiClient.post<{ message: string }>(`/dynamodb/${tableName}/items`, {
				item,
			}),
		onSuccess: () => {
			queryClient.invalidateQueries({
...
export function useDeleteItem(tableName: string) {
	...
	return useMutation({
		mutationFn: (key: Record<string, unknown>) =>
			apiClient.delete<{ success: boolean }>(`/dynamodb/${tableName}/items`, {
				key,
			}),
		onSuccess: () => {
			queryClient.invalidateQueries({
...
export function useBatchWriteItems(tableName: string) {
	...
	return useMutation({
		mutationFn: (data: {
			...
			apiClient.post<BatchWriteResponse>(
				`/dynamodb/${tableName}/items/batch-write`,
				data,
			),
		onSuccess: () => {
			queryClient.invalidateQueries({
...
export function useBatchGetItems(tableName: string) {
	return useMutation({
		mutationFn: (data: {
			...
			apiClient.post<BatchGetResponse>(
				`/dynamodb/${tableName}/items/batch-get`,
				data,
			),
	});
}
...
export function useDescribeStream(tableName: string) {
	return useQuery({
		...
		queryFn: () =>
			apiClient.get<StreamDescription>(`/dynamodb/${tableName}/streams`),
		enabled: !!tableName,
	});
}
...
export function useGetStreamRecords(tableName: string) {
	return useMutation({
		mutationFn: (params?: { shardId?: string; limit?: number }) => {
			...
			return apiClient.get<StreamRecordsResponse>(
				`/dynamodb/${tableName}/streams/records`,
				queryParams,
			);
		},
	});
