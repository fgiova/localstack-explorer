...
export function useTopicAttributes(topicName: string) {
	return useQuery({
		...
		queryFn: () =>
			apiClient.get<TopicDetailResponse>(`/sns/${topicName}/attributes`),
		enabled: !!topicName,
	});
}
...
export function useTopicSubscriptions(topicName: string) {
	return useQuery({
		...
		queryFn: () =>
			apiClient.get<SubscriptionListResponse>(
				`/sns/${topicName}/subscriptions`,
			),
		enabled: !!topicName,
	});
}
...
export function usePublishBatch(topicName: string) {
	...
	return useMutation({
		mutationFn: (request: PublishBatchRequest) =>
			apiClient.post<PublishBatchResponse>(
				`/sns/${topicName}/publish-batch`,
				request,
			),
		onSuccess: () => {
			queryClient.invalidateQueries({
