...
export function useListAccessKeys(userName: string) {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<ListAccessKeysResponse>(`/iam/users/${userName}/access-keys`),
    enabled: !!userName,
  });
}
...
export function useListUserInlinePolicies(userName: string) {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<InlinePolicyNamesResponse>(`/iam/users/${userName}/inline-policies`),
    enabled: !!userName,
  });
}
...
export function useListUserGroups(userName: string) {
  return useQuery({
    ...
    queryFn: () => apiClient.get<ListGroupsResponse>(`/iam/users/${userName}/groups`),
    enabled: !!userName,
  });
}
...
export function useGetGroup(groupName: string) {
  return useQuery({
    ...
    queryFn: () => apiClient.get<GroupDetailResponse>(`/iam/groups/${groupName}`),
    enabled: !!groupName,
  });
}
...
export function useCreateAccessKey() {
  ...
  return useMutation({
    mutationFn: (userName: string) =>
      apiClient.post<CreateAccessKeyResponse>(`/iam/users/${userName}/access-keys`),
...
export function useUpdateAccessKey() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: { userName: string; accessKeyId: string; status: string }) =>
...
export function usePutUserInlinePolicy() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: { userName: string; policyName: string; policyDocument: string }) =>
      apiClient.put<MessageResponse>(
...
export function useAddUserToGroup() {
  ...
  return useMutation({
    mutationFn: (data: { groupName: string; userName: string }) =>
      apiClient.post<MessageResponse>(
        `/iam/groups/${data.groupName}/members`,
...
export function usePutGroupInlinePolicy() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: { groupName: string; policyName: string; policyDocument: string }) =>
      apiClient.put<MessageResponse>(
  ...
  return useMutation({
    mutationFn: (data: {
      ...
      apiClient.post<MessageResponse>(
        ...
        { policyDocument: data.policyDocument, setAsDefault: data.setAsDefault }
      ),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
