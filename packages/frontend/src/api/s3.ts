...
export function useCreateBucket() {
	...
	return useMutation({
		mutationFn: (name: string) =>
			apiClient.post<{ message: string }>("/s3", { name }),
		onSuccess: () => {
...
export function useDeleteBucket() {
	...
	return useMutation({
		mutationFn: (bucketName: string) =>
			apiClient.delete<{ success: boolean }>(`/s3/${bucketName}`),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["s3", "buckets"] });
		},
	});
...
export function useObjectProperties(bucketName: string, key: string) {
	return useQuery({
		...
		queryFn: () =>
			apiClient.get<ObjectProperties>(`/s3/${bucketName}/objects/properties`, {
				key,
			}),
...
export function useUploadObject(bucketName: string) {
	...
	return useMutation({
		mutationFn: ({ file, key }: { file: File; key?: string }) => {
			...
			return apiClient.upload<{ key: string; bucket: string }>(
				`/s3/${bucketName}/objects/upload`,
				formData,
			);
		},
		onSuccess: () => {
	...
	return useMutation({
		mutationFn: (key: string) =>
			apiClient.delete<{ success: boolean }>(
				`/s3/${bucketName}/objects?key=${encodeURIComponent(key)}`,
			),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["s3", "objects", bucketName],
