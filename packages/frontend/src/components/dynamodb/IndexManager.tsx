...
import { useDescribeTable, useDeleteGSI } from "@/api/dynamodb";
        ...
        <CardContent>
          {gsis.length === 0 ? (
            <div className="py-8 text-center text-muted-foreground">
              No Global Secondary Indexes. Create one to enable additional query patterns.
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {gsis.map((gsi) => (
                  <TableRow key={gsi.indexName}>
                    <TableCell className="font-medium">{gsi.indexName}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge variant="secondary">{gsi.projection.projectionType}</Badge>
                    </TableCell>
                    <TableCell>
                      <Badge variant={gsi.indexStatus === "ACTIVE" ? "default" : "secondary"}>
                        {gsi.indexStatus ?? "\u2014"}
                      </Badge>
                    </TableCell>
              ...
              <TableBody>
                {lsis.map((lsi) => (
                  <TableRow key={lsi.indexName}>
                    <TableCell className="font-medium">{lsi.indexName}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge variant="secondary">{lsi.projection.projectionType}</Badge>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete index &quot;{deleteTarget}&quot;? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
