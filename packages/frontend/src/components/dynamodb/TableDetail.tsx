    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            ...
            <AttributeItem label="Table Size" value={formatBytes(table.tableSizeBytes)} />
            <AttributeItem
              label="Created"
              value={table.creationDateTime ? new Date(table.creationDateTime).toLocaleString() : undefined}
            />
            <AttributeItem
              label="Read Capacity"
            ...
            <AttributeItem
              label="Stream"
              value={table.streamSpecification?.streamEnabled ? table.streamSpecification.streamViewType ?? "Enabled" : "Disabled"}
            />
          </div>
        </CardContent>
      </Card>
              ...
              {table.keySchema.map((ks) => {
                ...
                return (
                  <TableRow key={ks.attributeName}>
                    <TableCell className="font-medium">{ks.attributeName}</TableCell>
                    <TableCell>
                      <Badge variant={ks.keyType === "HASH" ? "default" : "secondary"}>
      ...
      {table.tableArn && (
        <Card>
          ...
          <CardContent>
            <p className="text-sm font-mono text-muted-foreground break-all">{table.tableArn}</p>
          </CardContent>
        </Card>
      )}
    </div>
