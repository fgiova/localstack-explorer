...
import { useListTables, useDeleteTable } from "@/api/dynamodb";
    ...
    <div className="space-y-4">
      ...
      {filteredTables.length === 0 ? (
        <div className="py-12 text-center text-muted-foreground">
          {data?.tables.length === 0 ? "No tables found. Create one to get started." : "No tables match your search."}
        </div>
      ) : (
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {filteredTables.map((table) => (
              <TableRow key={table.tableName}>
                ...
                <TableCell>
                  <Badge variant={table.tableStatus === "ACTIVE" ? "default" : "secondary"}>
                    {table.tableStatus}
                  </Badge>
                </TableCell>
                ...
                <TableCell className="text-muted-foreground">
                  {table.tableSizeBytes !== undefined ? formatBytes(table.tableSizeBytes) : "\u2014"}
                </TableCell>
                <TableCell>
                  <Button
                    variant="ghost"
