...
export function ItemBrowser({ tableName }: ItemBrowserProps) {
  ...
  const handleLoadMore = () => {
    if (!lastKey) return;
    scanItems.mutate({ exclusiveStartKey: lastKey }, {
      onSuccess: (data: ItemsResponse) => {
  ...
  const extractKey = (item: Record<string, unknown>): Record<string, unknown> => {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Badge variant="secondary">
            {scanCount.count} items
          </Badge>
          {scanCount.scannedCount > scanCount.count && (
            <Badge variant="outline">
              {scanCount.scannedCount} scanned
            </Badge>
          )}
        </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  {columns.map((col) => (
                    <TableHead key={col}>
                      {tableDetail?.keySchema.some((ks) => ks.attributeName === col) ? (
      ...
      <Dialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <DialogContent>
          ...
          <pre className="rounded-md bg-muted p-4 text-sm overflow-auto max-h-[200px]">
            {deleteTarget ? JSON.stringify(extractKey(deleteTarget), null, 2) : ""}
          </pre>
          <DialogFooter>
