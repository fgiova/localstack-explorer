...
function EventBadge({ eventName }: { eventName?: string }) {
  const variant = eventName === "INSERT" ? "default" :
    eventName === "MODIFY" ? "secondary" :
    eventName === "REMOVE" ? "destructive" : "outline";
...
export function StreamViewer({ tableName }: StreamViewerProps) {
  ...
  const handleFetchRecords = (shardId?: string) => {
    getRecords.mutate({ shardId, limit: 100 }, {
      onSuccess: (data) => {
        ...
        <CardContent className="py-12 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            ...
            {streamInfo?.streamViewType && (
              <> Current view type: <Badge variant="outline">{streamInfo.streamViewType}</Badge></>
            )}
          </p>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="flex flex-col gap-1">
              ...
              <Badge variant={streamInfo.streamStatus === "ENABLED" ? "default" : "secondary"}>
                {streamInfo.streamStatus ?? "\u2014"}
              </Badge>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium text-muted-foreground">View Type</span>
              <span className="text-sm font-semibold">{streamInfo.streamViewType ?? "\u2014"}</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium text-muted-foreground">Label</span>
              <span className="text-sm font-semibold">{streamInfo.streamLabel ?? "\u2014"}</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium text-muted-foreground">Shards</span>
              <span className="text-sm font-semibold">{streamInfo.shards?.length ?? 0}</span>
            </div>
          </div>
            ...
            <div className="space-y-2">
              {streamInfo.shards.map((shard) => (
                ...
                >
                  ...
                  >
                    <RefreshCw className={`mr-2 h-3 w-3 ${getRecords.isPending ? "animate-spin" : ""}`} />
                    Fetch Records
                  </Button>
                </div>
              ))}
          ...
          <CardContent>
            <div className="space-y-3">
              {records.map((record, idx) => (
                <div key={record.eventID ?? idx} className="rounded-md border p-4 space-y-2">
                  ...
                  <JsonView label="New Image" data={record.dynamodb?.newImage} />
                  <JsonView label="Old Image" data={record.dynamodb?.oldImage} />
                </div>
              ))}
            </div>
          </CardContent>
