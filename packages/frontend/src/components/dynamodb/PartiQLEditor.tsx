...
const EXAMPLES = [
  ...
  { label: "Select with condition", template: (t: string) => `SELECT * FROM "${t}" WHERE pk = 'value'` },
  { label: "Insert", template: (t: string) => `INSERT INTO "${t}" VALUE {'pk': 'id1', 'data': 'example'}` },
  { label: "Update", template: (t: string) => `UPDATE "${t}" SET data='updated' WHERE pk='id1'` },
  { label: "Delete", template: (t: string) => `DELETE FROM "${t}" WHERE pk='id1'` },
];
    ...
    executePartiQL.mutate(
      { statement: statement.trim() },
      {
        onSuccess: (data) => {
          ...
          setHistory((prev) => [
            { statement: statement.trim(), timestamp: new Date(), success: true, itemCount: data.items.length },
            ...prev.slice(0, 9),
          ]);
        },
        onError: () => {
          setHistory((prev) => [
            { statement: statement.trim(), timestamp: new Date(), success: false },
            ...prev.slice(0, 9),
          ]);
        },
      }
