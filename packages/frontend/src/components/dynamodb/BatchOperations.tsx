...
export function BatchOperations({ tableName }: BatchOperationsProps) {
	...
	const [writeResult, setWriteResult] = useState<BatchWriteResponse | null>(
		null,
	);
					...
					<CardContent className="space-y-4">
						...
						{batchWrite.isError && (
							<p className="text-sm text-destructive">
								{batchWrite.error.message}
							</p>
						)}

						<Button onClick={handleBatchWrite} disabled={batchWrite.isPending}>
			...
			{activeTab === "get" && (
				<Card>
					...
					<CardContent className="space-y-4">
						...
						{batchGet.isError && (
							<p className="text-sm text-destructive">
								{batchGet.error.message}
							</p>
						)}

						<div className="flex gap-2">
							<Button onClick={handleBatchGet} disabled={batchGet.isPending}>
								<Play className="mr-2 h-4 w-4" />
											...
											<TableBody>
												{getResult.items.map((item, idx) => (
													...
													<TableRow key={idx}>
														{Array.from(
															new Set(
																getResult.items.flatMap((i) => Object.keys(i)),
															),
														).map((col) => (
															<TableCell
																key={col}
