				...
				<Table>
					...
					<TableBody>
						{subscriptions.map((sub) => {
							const queueName =
								sub.protocol === "sqs"
									? extractQueueNameFromArn(sub.endpoint)
									: null;
							const colorClass =
								protocolColors[sub.protocol] ??
								"bg-gray-100 text-gray-800 border-gray-200";

							return (
								<TableRow key={sub.subscriptionArn}>
									<TableCell>
										<Badge className={colorClass}>{sub.protocol}</Badge>
									</TableCell>
...
function SubscriptionAttributesDialog({
	subscriptionArn,
	open,
	onOpenChange,
}: {
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-lg">
				<DialogHeader>
					...
					<DialogDescription>Details for this subscription.</DialogDescription>
				</DialogHeader>
				{isLoading ? (
					...
					<div className="space-y-3 py-2 max-h-[60vh] overflow-y-auto">
						{[
							...
							{ label: "Owner", value: sub.owner },
							{
								...
								value: sub.rawMessageDelivery ? "Yes" : "No",
							},
							{
								label: "Confirmation Authenticated",
								value: sub.confirmationWasAuthenticated ? "Yes" : "No",
							},
							{
								label: "Pending Confirmation",
								value: sub.pendingConfirmation ? "Yes" : "No",
							},
							{ label: "Filter Policy Scope", value: sub.filterPolicyScope },
							{ label: "Filter Policy", value: sub.filterPolicy, isJson: true },
							{
								label: "Delivery Policy",
								value: sub.deliveryPolicy,
								isJson: true,
							},
							{
								label: "Effective Delivery Policy",
								value: sub.effectiveDeliveryPolicy,
								isJson: true,
							},
