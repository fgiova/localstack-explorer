...
export function EventsTimeline({ stackName, stackStatus }: EventsTimelineProps) {
        ...
        <TableBody>
          {events.map((event) => (
            <TableRow key={event.eventId}>
              ...
              <TableCell className="font-medium">{event.logicalResourceId ?? "\u2014"}</TableCell>
              <TableCell className="text-muted-foreground text-sm">{event.resourceType ?? "\u2014"}</TableCell>
              <TableCell>
              ...
              <TableCell className="text-sm text-muted-foreground max-w-[300px] truncate" title={event.resourceStatusReason ?? ""}>
