...
function ResourceLink({ resource }: { resource: Resource }) {
  ...
  switch (type) {
    case "AWS::S3::Bucket":
      return (
        <Link to="/s3/$bucketName" params={{ bucketName: physicalId }} className="text-primary underline hover:no-underline">
          {physicalId}
        </Link>
      );
    case "AWS::SQS::Queue":
      return (
        <Link to="/sqs/$queueName" params={{ queueName: extractQueueName(physicalId) }} className="text-primary underline hover:no-underline">
          {physicalId}
        </Link>
      );
    case "AWS::SNS::Topic":
      return (
        <Link to="/sns/$topicName" params={{ topicName: extractTopicName(physicalId) }} className="text-primary underline hover:no-underline">
          {physicalId}
        </Link>
      );
    case "AWS::IAM::User":
