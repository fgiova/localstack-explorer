...
import { useGetStack, useDeleteStack } from "@/api/cloudformation";
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                  <div className="flex flex-col gap-1 rounded-lg border p-4">
                    <span className="text-xs font-medium text-muted-foreground">Stack Name</span>
                    <span className="text-lg font-semibold">{stack.stackName}</span>
                  </div>
                  <div className="flex flex-col gap-1 rounded-lg border p-4">
                    <span className="text-xs font-medium text-muted-foreground">Stack ID</span>
                    <span className="text-sm font-semibold break-all">{stack.stackId ?? "\u2014"}</span>
                  </div>
                  <div className="flex flex-col gap-1 rounded-lg border p-4">
                    <span className="text-xs font-medium text-muted-foreground">Status</span>
                    <span className="text-lg font-semibold">{stack.status}</span>
                  </div>
                  <div className="flex flex-col gap-1 rounded-lg border p-4">
                    <span className="text-xs font-medium text-muted-foreground">Description</span>
                    <span className="text-lg font-semibold">{stack.description || "\u2014"}</span>
                  </div>
                  <div className="flex flex-col gap-1 rounded-lg border p-4">
                    <span className="text-xs font-medium text-muted-foreground">Creation Time</span>
                    <span className="text-lg font-semibold">{formatTimestamp(stack.creationTime)}</span>
                  </div>
                  <div className="flex flex-col gap-1 rounded-lg border p-4">
                    <span className="text-xs font-medium text-muted-foreground">Last Updated Time</span>
                    <span className="text-lg font-semibold">{formatTimestamp(stack.lastUpdatedTime)}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
                    ...
                    <TableBody>
                      {stack.outputs.map((output, idx) => (
                        <TableRow key={output.outputKey ?? idx}>
                          <TableCell className="font-medium">{output.outputKey ?? "\u2014"}</TableCell>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong>{stackName}</strong>? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
