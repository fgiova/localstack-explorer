...
function createTestRouter() {
  ...
  const history = createMemoryHistory({ initialEntries: [`/sqs/${TEST_QUEUE_NAME}`] });
...
describe("QueueDetail", () => {
  ...
  it("should call purge mutation when purge is confirmed", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("button", { name: /purge queue/i })).toBeInTheDocument();
    });
    ...
    const purgeButtons = screen.getAllByRole("button", { name: /purge queue/i });
