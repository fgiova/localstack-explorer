...
describe("QueueList", () => {
  ...
  it("should render queue list with data", async () => {
    mockUseListQueues.mockReturnValue({
      data: {
        queues: [
          { queueName: "my-queue", queueUrl: "http://localhost:4566/000000000000/my-queue" },
          { queueName: "other-queue", queueUrl: "http://localhost:4566/000000000000/other-queue" },
        ],
      },
      isLoading: false,
      error: null,
  ...
  it("should filter queues by search term", async () => {
    mockUseListQueues.mockReturnValue({
      data: {
        queues: [
          { queueName: "alpha-queue", queueUrl: "http://localhost:4566/000000000000/alpha-queue" },
          { queueName: "beta-queue", queueUrl: "http://localhost:4566/000000000000/beta-queue" },
        ],
      },
      isLoading: false,
      error: null,
