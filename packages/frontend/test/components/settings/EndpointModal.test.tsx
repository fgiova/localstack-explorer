...
describe("EndpointModal", () => {
  ...
  it("input shows the current endpoint value", () => {
    ...
    const input = screen.getByPlaceholderText("http://localhost:4566") as HTMLInputElement;
  ...
  it("Test Connection button triggers fetch to /api/health", async () => {
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ connected: true, endpoint: "http://localhost:4566", region: "us-east-1" }),
    });
  ...
  it("shows error message when test connection fails", async () => {
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ connected: false, endpoint: "http://localhost:4566", region: "us-east-1", error: "Connection refused" }),
    });
    vi.stubGlobal("fetch", mockFetch);
  ...
  it("Cancel button calls dismissModal", () => {
    useConfigStore.setState({ endpointModalOpen: true, userDismissedModal: false });
    renderWithProviders();
  ...
  it("Save button saves endpoint and closes modal after successful test", async () => {
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ connected: true, endpoint: "http://localhost:9999", region: "us-east-1" }),
    });
    vi.stubGlobal("fetch", mockFetch);
