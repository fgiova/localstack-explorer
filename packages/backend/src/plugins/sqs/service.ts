...
export class SQSService {
  ...
  async getQueueUrl(name: string): Promise<string> {
    try {
      ...
      if (error.name === "QueueDoesNotExist" || error.name === "NonExistentQueue") {
  ...
  async createQueue(name: string) {
    ...
    try {
      const response = await this.client.send(new CreateQueueCommand({ QueueName: name }));
      return { message: `Queue '${name}' created successfully`, queueUrl: response.QueueUrl };
    } catch (err) {
      const error = err as Error & { name: string };
      if (error.name === "QueueAlreadyExists") {
        throw new AppError(`Queue '${name}' already exists`, 409, "QUEUE_EXISTS");
      }
      throw error;
    }
  }
  ...
  async deleteQueue(queueUrl: string) {
    try {
      ...
      if (error.name === "NonExistentQueue") {
        throw new AppError(`Queue '${queueUrl}' not found`, 404, "QUEUE_NOT_FOUND");
      }
      throw error;
    }
  }
  ...
  async getQueueDetail(queueName: string) {
    ...
    try {
      ...
      return {
        ...
        receiveMessageWaitTimeSeconds: Number(attrs.ReceiveMessageWaitTimeSeconds ?? 0),
      };
    } catch (err) {
      const error = err as Error & { name: string };
      if (error.name === "NonExistentQueue") {
  ...
  async sendMessage(
    ...
    const attrs: Record<string, MessageAttributeValue> | undefined = messageAttributes
      ? Object.fromEntries(
