...
import type { CreateTableBody, ScanBody, QueryBody, CreateGSIBody } from "./schemas.js";
...
export class DynamoDBService {
    ...
    try {
      ...
      if (params.globalSecondaryIndexes) {
        commandInput.GlobalSecondaryIndexes =
          params.globalSecondaryIndexes.map((gsi) => ({
            IndexName: gsi.indexName,
            KeySchema: gsi.keySchema.map((ks) => ({
              AttributeName: ks.attributeName,
              KeyType: ks.keyType,
      ...
      if (params.localSecondaryIndexes) {
        commandInput.LocalSecondaryIndexes =
          params.localSecondaryIndexes.map((lsi) => ({
            IndexName: lsi.indexName,
            KeySchema: lsi.keySchema.map((ks) => ({
              AttributeName: ks.attributeName,
              KeyType: ks.keyType,
    ...
    try {
      const response = await this.docClient.send(
        new ScanCommand({
          ...
          ExpressionAttributeValues: options?.expressionAttributeValues as Record<string, any> | undefined,
          Limit: options?.limit,
          // biome-ignore lint/suspicious/noExplicitAny: AWS SDK NativeAttributeValue type
          ExclusiveStartKey: options?.exclusiveStartKey as Record<string, any> | undefined,
  ...
  async batchWriteItems(
    ...
    try {
      // biome-ignore lint/suspicious/noExplicitAny: AWS SDK NativeAttributeValue type
      const requests: Array<{ PutRequest?: { Item: Record<string, any> }; DeleteRequest?: { Key: Record<string, any> } }> = [];

      if (putItems) {
        for (const item of putItems) {
          // biome-ignore lint/suspicious/noExplicitAny: AWS SDK NativeAttributeValue type
          requests.push({ PutRequest: { Item: item as Record<string, any> } });
        }
      }
      if (deleteKeys) {
        for (const key of deleteKeys) {
          // biome-ignore lint/suspicious/noExplicitAny: AWS SDK NativeAttributeValue type
          requests.push({ DeleteRequest: { Key: key as Record<string, any> } });
        }
      }
          ...
          GlobalSecondaryIndexUpdates: [
            {
              Create: {
                ...
                Projection: {
                  ProjectionType: params.projection.projectionType as "ALL" | "KEYS_ONLY" | "INCLUDE",
  ...
  async getStreamRecords(tableName: string, shardId?: string, limit?: number) {
    try {
      ...
      const streamArn = tableDesc.Table?.LatestStreamArn;
      if (!streamArn) {
        throw new AppError("No stream enabled for this table", 404, "STREAM_NOT_FOUND");
      }
        ...
        dynamodb: record.dynamodb
          ? {
              keys: record.dynamodb.Keys
                ? Object.fromEntries(
                    Object.entries(record.dynamodb.Keys).map(([k, v]) => [k, Object.values(v)[0]])
                  )
                : undefined,
              newImage: record.dynamodb.NewImage
                ? Object.fromEntries(
                    Object.entries(record.dynamodb.NewImage).map(([k, v]) => [k, Object.values(v)[0]])
                  )
                : undefined,
              oldImage: record.dynamodb.OldImage
                ? Object.fromEntries(
                    Object.entries(record.dynamodb.OldImage).map(([k, v]) => [k, Object.values(v)[0]])
                  )
                : undefined,
  ...
  async executePartiQL(statement: string, parameters?: unknown[]) {
    try {
      const commandInput: ConstructorParameters<typeof ExecuteStatementCommand>[0] = {
        Statement: statement,
      };
